...
#define TWAI_TIMING_CONFIG_1KBITS()                                                \
    {                                                                              \
        .brp = 4000, .tseg_1 = 15, .tseg_2 = 4, .sjw = 3, .triple_sampling = false \
    }
#define TWAI_TIMING_CONFIG_5KBITS()                                               \
    {                                                                             \
        .brp = 800, .tseg_1 = 15, .tseg_2 = 4, .sjw = 3, .triple_sampling = false \
    }
#define TWAI_TIMING_CONFIG_10KBITS()                                              \
    {                                                                             \
        .brp = 400, .tseg_1 = 15, .tseg_2 = 4, .sjw = 3, .triple_sampling = false \
    }
#endif
#if (SOC_TWAI_BRP_MAX > 128) || (CONFIG_ESP32_REV_MIN >= 2)
#define TWAI_TIMING_CONFIG_12_5KBITS()                                            \
    {                                                                             \
        .brp = 256, .tseg_1 = 16, .tseg_2 = 8, .sjw = 3, .triple_sampling = false \
    }
#define TWAI_TIMING_CONFIG_16KBITS()                                              \
    {                                                                             \
        .brp = 200, .tseg_1 = 16, .tseg_2 = 8, .sjw = 3, .triple_sampling = false \
    }
    ...
    typedef struct
    {
        union
        {
            struct
            {
                ...
                uint32_t self : 1;         /**< Transmit as a Self Reception Request. Unused for received.作为自我接收请求发送。未用于接收。 */
                uint32_t dlc_non_comp : 1; /**< Message's Data length code is larger than 8. This will break compliance with ISO 11898-1 */
                uint32_t reserved : 27;    /**< Reserved bits */
            };
            // Todo: Deprecate flags待办事项:弃用标志
            uint32_t flags; /**< Deprecated: Alternate way to set bits using message flags已弃用:使用消息标志设置位的替代方法 */
        };
